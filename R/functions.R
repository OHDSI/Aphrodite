...
getPatientDataFromStartDate <- function (connection, dbms, patient_ids, patIndexDate, keywords, flags, schema, removeDomains=c('')) {
    ...
    for (patientQueue in 1:(length(patient_ids))) {
        ...
        # get patient dates
        patients_list_df[[1]] <- patIndexDate[patientQueue] #executeSQL(connection, schema, paste("SELECT person_id, observation_date FROM @cdmSchema.observation WHERE person_id=",as.character(patient_ids[patientQueue]),";",sep=''),dbms)
        patients_list_df[[2]] <- patIndexDate[patientQueue] #executeSQL(connection, schema, paste("SELECT person_id, condition_start_date AS observation_date FROM @cdmSchema.condition_occurrence WHERE person_id=",as.character(patient_ids[patientQueue]),";",sep=''),dbms)
        dates <- do.call(rbind, patients_list_df)
        remove('patients_list_df')
